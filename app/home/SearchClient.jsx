...
export default function SearchClient() {
  ...
  async function doSearch(query) {
    if (!query || query.length < 2) {
      setErr(query ? "Query minimal 2 karakter" : "");
      return;
    }
    setLoading(true);
    setErr("");
    try {
      const res = await searchBugs({ q: query, limit: 25, dev_limit: 10 });
      setData(res);
    } catch (e) {
  ...
  return (
    <main className="w-full min-h-screen bg-gray-100">
      ...
      <div className="max-w-7xl h-full  grid grid-cols-12 gap-4">
        <aside className=" col-span-12 md:col-span-3 bg-white border border-[#e4e4e4]  p-4">
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <button
              className="ml-auto bg-[#0D5DB8] text-white text-sm px-4 py-2 rounded"
              onClick={() => doSearch(q)}
              disabled={loading}
            >
        ...
        <section className="col-span-12 md:col-span-9">
          <div className="bg-white mt-6 text-sm">
            <a className="text-[#0D5DB8] hover:underline" href="#">
              Graph Bug
            </a>
